...
metadata <- read_csv(paste0("data/uploads/", name, "_metadata.csv")) %>%
  dplyr::filter(campaignid %in% "2021-05_Abrolhos_stereo-BRUVs") %>%
  glimpse()

# read in forwards annotations
forwards <- read.delim("data/raw/ga upload/BRUVS/habitat/2021-05_Abrolhos_stereo-BRUVs_Forwards_Dot Point Measurements.txt", 
...
# read in forwards annotations
backwards <- read.delim("data/raw/ga upload/BRUVS/habitat/2021-05_Abrolhos_stereo-BRUVs_Backwards_Dot Point Measurements.txt", 
                        header = T, skip = 4, stringsAsFactors = FALSE, 
                        colClasses = "character", na.strings = "") %>%
  clean_names()%>%
  dplyr::mutate(caab_code = as.numeric(caab_code))%>%
...
write_csv(tidy_habitat, "data/uploads/2021-05_Abrolhos_stereo-BRUVs_benthos-count.csv")


# RELIEF ----
# read in forwards annotations
forwards_relief <- read.delim("data/raw/ga upload/BRUVS/habitat/2021-05_Abrolhos_stereo-BRUVs_Forwards_Relief_Dot Point Measurements.txt", 
                              header = T, skip = 4, stringsAsFactors = FALSE, 
                              colClasses = "character", na.strings = "") %>%
  clean_names() %>%
  glimpse
...
# read in forwards annotations
backwards_relief <- read.delim("data/raw/ga upload/BRUVS/habitat/2021-05_Abrolhos_stereo-BRUVs_Backwards_Relief_Dot Point Measurements.txt", 
                               header = T, skip = 4, stringsAsFactors = FALSE, 
                               colClasses = "character", na.strings = "") %>%
  clean_names() %>%
  glimpse
...
write_csv(tidy_relief, "data/uploads/2021-05_Abrolhos_stereo-BRUVs_benthos-relief.csv")
