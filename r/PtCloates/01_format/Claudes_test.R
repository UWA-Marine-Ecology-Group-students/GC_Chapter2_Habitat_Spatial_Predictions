...
# Study name ----
study<-"2021-05_PtCloates_stereo-BRUVs"  # enter your study name here
...
# Read in metadata----
metadata <- read_csv("em_export/2021-05_PtCloates_stereo-BRUVS_Metadata.csv") %>% # read in the file
...
points <- read.delim("2021-05_PtCloates_stereo_BRUVS_Forwards_Dot Point Measurements.txt",header=T,skip=4,stringsAsFactors=FALSE) %>% # read in the file
  ga.clean.names() %>% # tidy the column names using GlobalArchive function
...
relief <- read.delim("2021-05_PtCloates_stereo_BRUVS_Forwards_Relief_Dot Point Measurements.txt",header=T,skip=4,stringsAsFactors=FALSE) %>% # read in the file
  ga.clean.names() %>% # tidy the column names using GlobalArchive function
  mutate(sample=str_replace_all(.$filename,c(".png"="",".jpg"="",".JPG"=""))) %>%
  mutate(sample=as.character(sample)) %>% 
