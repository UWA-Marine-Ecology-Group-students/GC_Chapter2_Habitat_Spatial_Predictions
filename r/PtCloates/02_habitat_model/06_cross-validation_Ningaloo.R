...
stack <- readRDS(paste(paste0('data/spatial/rasters/', name),      # This is ignored - too big!
  ...
  crop(ext(113.4, 113.7, -22.85, -22.60)) %>%
  brick() 
...
saveRDS(modpred, "data/tidy/PtCloates_modpreds.rds")                           # Save out to stop re running mega loop -- ignored
...
modpred <- readRDS("data/tidy/PtCloates_modpreds.rds")
...
predhabdf <- readRDS(paste0("output/PtCloates/", name, "_spatial_habitat_predictions.rds")) %>%   # Ignored
...
roc.dat$predicted <- apply(roc.dat %>% dplyr::select(sand, inverts), 1, 
